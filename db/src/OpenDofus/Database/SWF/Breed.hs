...
loadBreeds :: FilePath -> IO [(Breed, V.Vector BreedCharacteristicCost, V.Vector BreedSpell)]
loadBreeds fp = do
  SWF.loadData fp $ \obj -> do
...
getBreed :: Int -> Value -> (Breed, V.Vector BreedCharacteristicCost, V.Vector BreedSpell)
  ...
  let str i = SWF.unsafeString i $ fromMaybe (error i) $ H.lookup (T.pack i) b
      ...
      breed =
        Breed
          ...
          (str "d")
      spells =
        BreedSpell (BreedPK (BreedId bid)) . SpellPK . SpellId . SWF.unsafeInt "spell" <$> arr "s"
      costs =
          getCharacteristic =<< V.fromList [10, 11, 12, 13, 14, 15]
