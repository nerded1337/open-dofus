...
loadItems itemsPath itemstatsPath = SWF.loadData itemstatsPath $ \statsObj ->
  do
    ...
    SWF.loadData itemsPath $ \obj -> do
      let
        (Object objects   ) = fromMaybe (error "I") $ H.lookup "I" obj
        ...
        superTypes'         = V.fromList $ H.elems $ H.mapWithKey
          (\k _ ->
            let stid = (fst <$> fromRight (error "Invalid k") $ T.decimal k)
            in  ItemSuperType stid
          )
          superTypes
        slots' = fold $ V.fromList $ H.elems $ H.mapWithKey
          (\k v ->
            let tid = (fst <$> fromRight (error "Invalid k") $ T.decimal k)
            in  getSlots tid v
          )
          slots
        types' = V.fromList $ H.elems $ H.mapWithKey
          (\k v ->
            let tid = (fst <$> fromRight (error "Invalid k") $ T.decimal k)
            in  getType tid v
          )
          types
        items' = V.fromList $ H.elems $ H.mapWithKey
          (\k v ->
