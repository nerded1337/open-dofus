  ...
  where
    go fileName = SWF.loadData (path <> "/" <> fileName) $ \mapInfos ->
      let get i = fromMaybe (error (show i)) $ H.lookup i mapInfos
          mdate =
            fromRight (error "Invalid map file name") $
              parseFileName $
                BS.pack
                  fileName
...
getMap ::
  Word32 ->
  (String, Word32, Word32, Int32, Int32, Int32, Bool, Word32, T.Text) ->
  Value ->
  Map
...
getSuperArea :: Int32 -> Value -> MapSuperArea
...
getSubArea :: Int32 -> Value -> (MapSubArea, V.Vector MapSubAreaNeighbour)
