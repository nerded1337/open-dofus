...
getMapsDate path =
  fold <$> (traverse go =<< listDirectory path)
  where
    go fileName = do
      SWF.loadData (path <> "/" <> fileName) $ \mapInfos -> do
        let get i = fromMaybe (error (show i)) $ H.lookup i mapInfos
            mdate = fromRight (error "Invalid map file name") $ parseFileName $ BS.pack fileName
...
getMap :: Int -> (String, Int, Int, Int, Int, Int, Bool, Int, T.Text) -> Value -> Map
...
getSuperArea suId (String x) = MapSuperArea (MapSuperAreaId suId) x
getSuperArea _ x             = error $ "Unhandled map super area: " <> show x

getArea :: Int -> Value -> MapArea
getArea aId (Object area) =
  MapArea
    (MapAreaId aId)
    (SWF.unsafeString "n" $ fromMaybe (error "n") $ H.lookup "n" area)
    (MapSuperAreaPK $
     MapSuperAreaId $
     SWF.unsafeInt "sua" $ fromMaybe (error "sua") $ H.lookup "sua" area)
...
getSubArea saId (Object subArea) =
  let subArea' =
        MapSubArea
          ...
          (MapAreaPK $ MapAreaId $ 
           SWF.unsafeInt "a" $ fromMaybe (error "a") $ H.lookup "a" subArea)
          mempty
      neighbours =
        MapSubAreaNeighbour (MapSubAreaPK (MapSubAreaId saId)) .
        MapSubAreaPK . MapSubAreaId <$>
        (SWF.unsafeInt "v" <$>
         (SWF.unsafeArray "v" $ fromMaybe (error "v") $ H.lookup "v" subArea))
