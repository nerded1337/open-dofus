...
import           OpenDofus.Prelude hiding             (First, getFirst)

loadEffects :: FilePath -> IO [Effect]
loadEffects filePath = do
  SWF.loadData filePath $ \obj -> do
    let (Object effects) = fromMaybe (error "E") $ H.lookup "E" obj
        (Array edmg) = fromMaybe (error "EDMG") $ H.lookup "EDMG" obj
        (Array ehel) = fromMaybe (error "EHEL") $ H.lookup "EHEL" obj
        etype eid =
          fromMaybe Special $
          getFirst $
          foldMap
            First
            [ const Damage <$> V.find ((== eid) . SWF.unsafeInt "") edmg
            , const Heal <$> V.find ((== eid) . SWF.unsafeInt "") ehel
            ]
    pure $
      H.elems $
      H.mapWithKey
...
getEffect eid etype (Object effect) =
  ...
  in Effect
       ...
       (SWF.unsafeString "description" $
        fromMaybe (error "description") $ H.lookup "d" effect)
       (fromMaybe False $ SWF.unsafeBool "hasJet" <$> H.lookup "j" effect)
       (fromMaybe False $ SWF.unsafeBool "hasJet" <$> H.lookup "t" effect)
       (getOperator =<< H.lookup "o" effect)
       (fromMaybe 0 $ SWF.unsafeInt "characteristic" <$> H.lookup "c" effect)
