...
-- deriving instance Eq Skill
-- deriving instance Show Skill

type SkillPK = PrimaryKey SkillT Identity

deriving instance Eq SkillPK

deriving instance Show SkillPK

Skill
  (LensFor skillId)
  (LensFor skillDescription)
  (JobPK (LensFor skillJobId))
  (InteractiveObjectPK (LensFor skillInteractiveObjectId))
  (ItemPK (LensFor skillItemId))
  (LensFor skillCriterion) =
    tableLenses
...
deriving instance Eq SkillCraft

deriving instance Show SkillCraft

type SkillCraftPK = PrimaryKey SkillCraftT Identity

deriving instance Eq SkillCraftPK

deriving instance Show SkillCraftPK

SkillCraft
  (SkillPK (LensFor skillCraftSkillId))
  (ItemPK (LensFor skillCraftItemId)) =
    tableLenses
